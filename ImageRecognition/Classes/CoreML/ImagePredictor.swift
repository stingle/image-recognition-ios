...
public class ImagePredictor {
    ...
    public init() {
        do {
            ...
            let mobileNet = try MobileNet(configuration: config)
            self.imageClassifier = try VNCoreMLModel(for: mobileNet.model)
        } catch {
    ...
    private func visionRequestHandler(_ request: VNRequest, error: Error?) {
        ...
        predictions = observations.map { observation in
            Prediction(classification: observation.identifier, confidencePercentage: observation.confidence * 100)
        }
    }
}
